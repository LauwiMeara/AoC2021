...
namespace _07._2
{
    class Program
    {
        static void Main()
        {
            ...
            int minFuel = positions.Sum(position => SumOfRange(position.Key) * position.Value);

            // Calculate the amount of fuel needed to align every crab to every position
            for (int i = 1; i <= positions.Keys.Max(); i++)
            {
                int fuel = positions.Sum(position => SumOfRange(Math.Abs(position.Key - i)) * position.Value);
